...
Function ulong LoadLibrary(Ref String lpszDllName) Library "kernel32" Alias for "LoadLibraryA"
Function boolean FreeLibrary(ulong hModule) Library "kernel32"
Function ulong GetModuleHandle(string lpModuleName) Library "kernel32.dll" Alias for "GetModuleHandleA"
Function ulong GetProcAddress(ulong hModule, String lpProcName) Library "kernel32"
...
Function long CreateWindowEx(ulong dwExStyle, string ClassName, long WindowName, ulong dwStyle, ulong X, ulong Y, ulong nWidth, ulong nHeight, ulong hWndParent, ulong hMenu, ulong hInstance, ulong lpParam) library "user32.dll" alias for "CreateWindowExA"
Function integer DestroyWindow(ulong hWnd) library "user32.dll"
Function integer ToolTipMsg(ulong hWnd, ulong uMsg, ulong wParam, REF TOOLINFO ToolInfo) library "user32" Alias For "SendMessageA"
Function integer RelayMsg(ulong hWnd, ulong uMsg, ulong wParam, REF MSG Msg) library "user32" Alias For "SendMessageA"
...
Function long lstrcpy(ulong Destination, string Source) library "kernel32" Alias For "lstrcpyA"

Function uLong SendMessageString( uLong hwnd, uLong Msg, uLong wParam, String lpzString ) Library "user32" Alias For "SendMessageA"

Function uLong CreateFontIndirect( Ref str_Logfont lplf ) Library 'gdi32' alias for "CreateFontIndirectA"
...
/* Wrapper for system ToolTips
	...
	Modified By Sebastien Kirche - 07-2013
...
Constant ulong TTM_ADDTOOLA			= WM_USER + 4
Constant ulong TTM_ADDTOOLW 		= WM_USER + 50
Constant ulong TTM_DELTOOLA			= WM_USER + 5
Constant ulong TTM_DELTOOLW 		= WM_USER + 51
Constant ulong TTM_NEWTOOLRECTA		= WM_USER + 6
Constant ulong TTM_NEWTOOLRECTW 	= WM_USER + 52
Constant ulong TTM_RELAYEVENT 		= WM_USER + 7
Constant ulong TTM_UPDATETIPTEXTA	= WM_USER + 12
...
public subroutine of_removealltools ()
end prototypes
...
If ToolTipMsg( hWndTT, TTM_ADDTOOLA, 0, ToolInfo ) = 0 Then
...
ToolTipMsg( hWndTT, TTM_NEWTOOLRECTA, 0, ToolInfo )
...
ToolTipMsg( hWndTT, TTM_UPDATETIPTEXTA, 0, ToolInfo )
...
ToolTipMsg( hWndTT, TTM_DELTOOLA, 0, ToolInfo )
...
public subroutine of_removealltools ();/*****************************************************************************
	...
	Function:		of_removealltools

	Description:	Unregisters all controls within the tooltip control
...
-------------------------------------------------------------------------------
	...
	--------------------
	Date			Version
	02-04-2017		1.0		Initial version
...
for i = 1 to upperbound(iul_Handle[])

	ToolInfo.hWnd		= iul_Handle[i]
	ToolInfo.uID		= i
	ToolTipMsg( hWndTT, TTM_DELTOOLA, 0, ToolInfo )
